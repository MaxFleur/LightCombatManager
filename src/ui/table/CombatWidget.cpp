...
CombatWidget::generateTableFromTableData()
{
    ...
    m_logListWidget->log(LogListWidget::LogType::CHANGED_CHAR_COUNT, m_characterHandler->getCharacters().size(), true);
    pushOnUndoStack();
...
CombatWidget::openAddCharacterDialog()
{
    ...
    connect(dialog, &AddCharacterDialog::characterCreated, this, [this, &sizeBeforeDialog] (CharacterHandler::Character character, int instanceCount) {
        m_logListWidget->log(LogListWidget::LogType::CHANGED_CHAR_COUNT, m_characterHandler->getCharacters().size() - sizeBeforeDialog, true);
...
CombatWidget::dragAndDrop(int /* logicalIndex */, int oldVisualIndex, int newVisualIndex)
{
    ...
    // Then set the table
    m_logListWidget->log(LogListWidget::LogType::CHARS_SWITCHED, oldVisualIndex + 1, newVisualIndex + 1);
    pushOnUndoStack();
...
CombatWidget::openStatusEffectDialog()
{
    ...
    if (auto *const dialog = new StatusEffectDialog(m_ruleSettings, this); dialog->exec() == QDialog::Accepted) {
        ...
        if (needsUndo) {
            m_logListWidget->log(LogListWidget::LogType::ADDED_EFFECT, m_tableWidget->selectionModel()->selectedRows().size());
            pushOnUndoStack();
        }
    }
...
CombatWidget::rerollIni()
{
    ...
    m_logListWidget->log(LogListWidget::LogType::STAT_CHANGED, row + 1, 1);
    pushOnUndoStack();
...
CombatWidget::changeHPForMultipleChars()
{
    ...
    if (auto *const dialog = new ChangeHPDialog(this); dialog->exec() == QDialog::Accepted) {
        ...
        m_logListWidget->log(LogListWidget::LogType::CHANGED_MULTIPLE_HP, m_tableWidget->selectionModel()->selectedRows().size());
        pushOnUndoStack();
    }
}
...
CombatWidget::removeRow()
{
    ...
    m_logListWidget->log(LogListWidget::LogType::CHANGED_CHAR_COUNT, m_removedOrAddedRowIndices.size(), false);
    setRowAndPlayer();
    pushOnUndoStack();
    m_tableWidget->itemSelectionChanged();
}
...
CombatWidget::duplicateRow()
{
    ...
    m_logListWidget->log(LogListWidget::LogType::DUPLICATED_CHAR, currentIndex + 1);
    pushOnUndoStack();
    m_tableWidget->itemSelectionChanged();
}
...
CombatWidget::handleTableWidgetItemPressed(QTableWidgetItem *item)
{
    if (item->column() == Utils::Table::COL_NAME) {
        ...
        m_logListWidget->log(LogListWidget::LogType::STAT_CHANGED, item->row(), 4, item->checkState() != Qt::Checked);
    ...
    if (tableData != m_tableDataOld || m_rowEnteredOld != m_rowEntered || m_roundCounterOld != m_roundCounter) {
        m_tableWidget->resynchronizeCharacters();
        m_logListWidget->log(LogListWidget::LogType::STAT_CHANGED, item->row(), item->column());
        pushOnUndoStack();
    }
}
...
CombatWidget::sortTable()
{
    ...
    m_rowEntered = 0;
    m_logListWidget->log("Table resorted.");
    pushOnUndoStack();
}
...
CombatWidget::switchCharacterPosition(bool goDown)
{
    ...
    setRowAndPlayer();
    m_logListWidget->log(LogListWidget::LogType::CHARS_SWITCHED, originalIndex + 1, goDown ? originalIndex + 2 : originalIndex);
    pushOnUndoStack();
...
CombatWidget::enteredRowChanged(bool isGoingDown)
{
    ...
    setRowAndPlayer();
    m_logListWidget->log(LogListWidget::LogType::NEXT_TURN, m_roundCounter, m_rowEntered + 1);
    pushOnUndoStack(true);
}
